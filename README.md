...
Otherwise, you can run the generation script locally. The file `webgpu.hpp` is generated by running the `generate.py` script (with at least Python 3.10). The best way to get an up to date doc is through it's help message:
...
# Dawn
python generate.py -u dawn/webgpu.h -t webgpu.template.hpp -o dawn/webgpu.hpp --use-init-macros
